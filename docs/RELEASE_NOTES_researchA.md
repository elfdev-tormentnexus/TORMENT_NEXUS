...
| labelled paraphrases, trace | 83% | 77% |

## What is measured and unflattering

- **The plain averaged vector beats the trace at naming a concept: 90%
  against 83%** on 30 labelled paraphrases. The trace's claim is *where* a
  meaning sat, not *which* it was, and only the second is tested here — but
  "trajectories shadow retrieval" is now a number rather than an
  impression.
- **The trace degrades as the dictionary grows** and the pooled read does
  not. It takes a max across token positions, so every added anchor is
