...
| Concept trace (`trace`) | Experimental; requires machinespirit | Local, no text stored in the trajectory container | Concepts come from a fixed anchor list, not from the model naming them. Identifies the right concept 83% of the time against the plain averaged vector's 90%, on 30 labelled paraphrases. |
