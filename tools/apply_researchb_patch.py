"""Apply a direct machinesoul researchB patch after exact verification.

Extends the researchA applier with one capability it deliberately lacked:
a patch may **add** a file that the base cut never contained. researchA
patches could only replace files the release manifest already named, and
`apply_researcha_patch.py` refuses a missing install path outright. That
is the safer rule when a patch only corrects existing behaviour, and it
is the wrong rule for a patch whose whole content is a new module.
...
def load_manifest(path: Path) -> dict:
    ...
    if document.get("format") != FORMAT:
        raise PatchError("not a researchB patch manifest")
    ...
    try:
        for item, installed, payload, state in checks:
            ...
            temporary = installed.with_name(
                installed.name + ".researchb-patch.tmp"
            )
